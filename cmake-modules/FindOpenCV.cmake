...
#  OpenCV (env. variable):Base directory of OpenCv tree to use.
#
## 2: Variable
...
if(NOT EXISTS "${OpenCV_DIR}")
    # Use OpenCV environment variable if no OpenCV_DIR variable has been defined locally
    if(EXISTS "$ENV{OpenCV}")
        set(OpenCV_DIR "$ENV{OpenCV}" CACHE STRING "" FORCE)
    # Otherwise, try to find OpenCVConfig.cmake using pkg-config if on linux
    else(EXISTS "$ENV{OpenCV}")
        if(NOT WIN32)
            include(FindPkgConfig)
            if(PKG_CONFIG_FOUND)
                pkg_check_modules(OPENCV_PKGCONF opencv)
                if(EXISTS "${OPENCV_PKGCONF_PREFIX}/share/OpenCV/OpenCVConfig.cmake")
                    set(OpenCV_DIR "${OPENCV_PKGCONF_PREFIX}/share/OpenCV/")
                endif(EXISTS "${OPENCV_PKGCONF_PREFIX}/share/OpenCV/OpenCVConfig.cmake")
...
if(EXISTS "${OpenCV_DIR}")
        ...
        set(ERR_MSG "Please specify OpenCV directory using OpenCV env. variable")
